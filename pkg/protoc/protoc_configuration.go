...
func newProtocConfiguration(pc *PackageConfig, lc *LanguageConfig, workDir, rel, prefix string, lib ProtoLibrary, plugins []*PluginConfiguration) *ProtocConfiguration {
	srcs, mappings := mergeSources(workDir, rel, plugins, pc.GetStripPathPrefix())
...
// necessary mappings.
func mergeSources(workDir, rel string, plugins []*PluginConfiguration, stripPrefix string) ([]string, map[string]string) {
	srcs := make([]string, 0)
	...
	for _, plugin := range plugins {
		...
		for _, filename := range plugin.Outputs {
			...
			if dir == "." && rel == "" {
				...
				// If we are asked to strip the prefix we need to trick the code below into thinking that it
				// needs to generate a mapping for the file. Because the generated one will never be in the
				// right place.
				if strings.HasPrefix(rel, stripPrefix[1:]) {
			...
			if dir == rel {
				...
				// If there is a prefix to strip the filename that will be generated won't have the prefix
				// because it will be based on the package name not the location in the folder so we need
				// to strip it too in the mapping otherwise bazel won't be able to find it
				if strings.HasPrefix(filename, stripPrefix[1:]) {
					filename = filename[len(stripPrefix):]
				}
